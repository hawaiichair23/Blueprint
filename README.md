...
Blueprint is an MCP server for accelerated development with AI. At its core is a simple declarative syntax that allows either the user or the AI assistant to write to a .txt file and run node generate.js to generate files locally. The parser is located in generate.js. It reads the blueprint txt files from blueprint/, parses its syntax, extracts the parameters from the semicolon-separated key value pairs, matches it with components from the schema/ library, and uses Node.js's `fs.writeFileSync()` to write generated HTML/CSS/JS directly to the filesystem in the sandbox.
...
bash   git clone https://github.com/yourusername/Blueprint.git
   cd Blueprint
...
Blueprint currently only works with Claude Desktop until OpenAI stops being bad. Note: Claude Desktop the app, not Claude in the browser.
...
```json
{
  "mcpServers": {
    "blueprint": {
      "command": "node",
      "args": ["C:\\Users\\YourName\\Desktop\\Blueprint\\server.js"] (wherever it is)
    }
  }
}
```
